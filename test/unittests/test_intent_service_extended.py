...
class TestMatchTimeout(unittest.TestCase):
    ...
    def test_hung_plugin_does_not_saturate_pool_for_healthy_plugins(self):
        ...
        svc = _make_service(config={"match_timeout": 0.1, "match_workers": 2})
        svc._dispatch_match = MagicMock()
        svc.send_complete_intent_failure = MagicMock()
    ...
    def test_single_flight_is_scoped_per_session(self):
        ...
        svc = _make_service(config={"match_timeout": 0.1, "match_workers": 4})
        dispatched = []
        svc._dispatch_match = MagicMock(side_effect=lambda m, *a, **kw: dispatched.append(m))
        svc.send_complete_intent_failure = MagicMock()
    ...
    def _run_as(self, svc, session_id, matchers):
        """Like ``_run`` but with a named session."""
        sess = Session(session_id)
        sess.pipeline = [m[0] for m in matchers]
        msg = Message("recognizer_loop:utterance",
                      data={"utterances": ["hello"]},
    ...
    def test_more_hung_sessions_than_workers_do_not_starve_a_fresh_session(self):
        """More hung sessions than the pool has workers. A fresh session
        must still reach a healthy plugin at once, and must be declined by
        the hung plugin at once instead of waiting a full bound in the
        queue. The hung sessions outnumber the largest default pool
        (``min(32, cpu + 4)``), so a single shared pool fails this test on
        every box."""
        ...
        try:
            for i in range(40):
                self._run_as(svc, f"hung-{i}", [("hung-plugin", hung)])
            # the 40 hung sessions each failed at their bound, as expected
            self.assertEqual(svc.send_complete_intent_failure.call_count, 40)
            svc.send_complete_intent_failure.reset_mock()

            started = time.monotonic()
            self._run_as(svc, "fresh", [("hung-plugin", hung), ("healthy-plugin", healthy)])
            elapsed = time.monotonic() - started
        finally:
            release.set()
        ...
        dispatched = svc._dispatch_match.call_args.args[0]
        self.assertEqual(dispatched.match_type, "healthy:intent")
        # the hung plugin declined the fresh session without a queue wait:
        # far under the 0.1s bound a queued future would have waited for
        self.assertLess(elapsed, 0.09)
        svc.send_complete_intent_failure.assert_not_called()

    def test_per_plugin_worker_budget_is_config(self):
        """``intents.<pipe_id>.match_workers`` sizes that plugin's pool."""
        svc = _make_service(config={"match_timeout": 0.1,
                                    "match_workers": 1,
                                    "wide-plugin": {"match_workers": 3}})
        svc._dispatch_match = MagicMock()
        release = threading.Event()
        ...
        try:
            self._run_as(svc, "s1", [("narrow-plugin", hung)])
            self._run_as(svc, "s1", [("wide-plugin", hung)])
            self.assertEqual(svc._match_busy_workers("narrow-plugin"), 1)
            self.assertEqual(svc._match_busy_workers("wide-plugin"), 1)
            with patch("ovos_core.intent_services.service.LOG") as log:
                self._run_as(svc, "s2", [("narrow-plugin", hung)])
                self._run_as(svc, "s2", [("wide-plugin", hung)])
            declined = [c.args[0] for c in log.warning.call_args_list
                        if "match workers held" in c.args[0]]
            self.assertEqual(len(declined), 1)
            self.assertIn("narrow-plugin", declined[0])
            self.assertEqual(svc._match_busy_workers("narrow-plugin"), 1)
            self.assertEqual(svc._match_busy_workers("wide-plugin"), 2)
        finally:
            release.set()

    def test_hung_high_tier_does_not_skip_medium_tier_in_the_same_round(self):
        """The single-flight key is the full pipeline entry. A ``-high``
        tier that outlives its bound must not make the ``-medium`` tier of
        the same plugin decline in the same round."""
        svc = _make_service(config={"match_timeout": 0.1})
        svc._dispatch_match = MagicMock()
        svc.send_complete_intent_failure = MagicMock()
        medium_match = _make_match(match_type="medium:intent", skill_id="m.skill")

        def slow_high(utterances, lang, message):
            time.sleep(0.5)

        def medium(utterances, lang, message):
            return medium_match

        self._run(svc, [("tier-plugin-high", slow_high),
                        ("tier-plugin-medium", medium)], bound_wait=0.5)

        svc._dispatch_match.assert_called_once()
        self.assertEqual(svc._dispatch_match.call_args.args[0].match_type,
                         "medium:intent")

    def test_timed_out_call_that_never_started_is_cancelled(self):
        """A call still in the queue when its bound expires is cancelled,
        so it does not run later on a stale message."""
        svc = _make_service(config={"match_timeout": 0.1, "match_workers": 1})
        release = threading.Event()
        ran = []

        def hung(utterances, lang, message):
            ran.append(message.context["session"]["session_id"])
            release.wait(10)

        def msg(session_id):
            return Message("x", context={"session": {"session_id": session_id}})

        try:
            with patch("ovos_core.intent_services.service.LOG"):
                svc._call_match_bounded("one-plugin", hung, [], "en-US", msg("s1"), "s1")
                # the budget check would decline s2 at once; bypass it so
                # the call queues behind s1 and reaches the cancel path
                with patch.object(svc, "_match_busy_workers", return_value=0):
                    svc._call_match_bounded("one-plugin", hung, [], "en-US", msg("s2"), "s2")
        finally:
            release.set()
            time.sleep(0.2)

        self.assertEqual(ran, ["s1"])
        self.assertTrue(svc._match_inflight[("one-plugin", "s2")].cancelled())

    @staticmethod
    def _threaded_patches(svc, matchers_by_session):
        """The patches for a run where several threads call
        handle_utterance at once. ``patch`` is not thread-safe, so the
        patches are entered once here, in the calling thread, and every
        worker thread runs inside them."""
        def get_pipeline(session=None):
            return matchers_by_session[session.session_id]
        return [patch.object(svc, "get_pipeline", side_effect=get_pipeline),
                patch("ovos_core.intent_services.service.SessionManager.sync"),
                patch("ovos_core.intent_services.service.get_message_lang",
                      return_value="en-US"),
                patch("ovos_core.intent_services.service.get_valid_languages",
    ...
    @staticmethod
    def _utterance(session_id, matchers):
        sess = Session(session_id)
        sess.pipeline = [m[0] for m in matchers]
        return Message("recognizer_loop:utterance",
                       data={"utterances": ["hello"]},
    ...
    def test_healthy_concurrent_sessions_are_never_declined(self):
        """Six sessions arrive 20 ms apart on one plugin whose call takes
        0.3 s, well inside the 10 s bound, with the default 4 workers.
        Bus handlers run concurrently, so this is a HiveMind hub with six
        users. A healthy call inside its bound is never a reason to
        decline: only workers held by calls that outlived their bound
        count. All six are dispatched, as on dev."""
        svc = _make_service(config={"match_timeout": 10})
        dispatched = []
        lock = threading.Lock()
        ...
        def record(m, *a, **kw):
            with lock:
                dispatched.append(m)
        svc._dispatch_match = MagicMock(side_effect=record)
        svc.send_complete_intent_failure = MagicMock()
        healthy_match = _make_match(match_type="healthy:intent", skill_id="healthy.skill")

        def slow_but_healthy(utterances, lang, message):
            time.sleep(0.3)
            return healthy_match

        matchers = {f"s{i}": [("one-plugin", slow_but_healthy)] for i in range(6)}
        threads = []
        with contextlib.ExitStack() as stack:
            for p in self._threaded_patches(svc, matchers):
                stack.enter_context(p)
            log = stack.enter_context(patch("ovos_core.intent_services.service.LOG"))
            for i in range(6):
                t = threading.Thread(target=svc.handle_utterance,
                                     args=(self._utterance(f"s{i}", matchers[f"s{i}"]),))
                t.start()
                threads.append(t)
                time.sleep(0.02)
            for t in threads:
                t.join()
        declines = [c.args[0] for c in log.warning.call_args_list
                    if "match workers" in c.args[0]]
        self.assertEqual(declines, [])
        self.assertEqual(len(dispatched), 6)
        svc.send_complete_intent_failure.assert_not_called()

    def test_hung_burst_never_starts_a_fifth_worker(self):
        """Eight sessions arrive in the same millisecond on a plugin that
        hangs, with 4 workers and a 0.2 s bound. A call is not known to
        be hung until its bound expires, so the burst queues; the four
        queued calls are cancelled at their bound and never start. Then
        every worker is held by a call past its bound, and a fresh
        session is declined at once, without a wait."""
        svc = _make_service(config={"match_timeout": 0.2, "match_workers": 4})
        svc._dispatch_match = MagicMock()
        svc.send_complete_intent_failure = MagicMock()
        release = threading.Event()
        started = []
        ...
        def hung(utterances, lang, message):
            with lock:
                started.append(message.context["session"]["session_id"])
            release.wait(10)

        matchers = {f"b{i}": [("burst-plugin", hung)] for i in range(8)}
        matchers["fresh"] = [("burst-plugin", hung)]
        barrier = threading.Barrier(8)

        def run(i):
            barrier.wait()
            svc.handle_utterance(self._utterance(f"b{i}", matchers[f"b{i}"]))
        ...
        try:
            with contextlib.ExitStack() as stack:
                ...
                self.assertEqual(svc._match_busy_workers("burst-plugin"), 4)
                with patch("ovos_core.intent_services.service.LOG") as log:
                    t0 = time.monotonic()
                    svc.handle_utterance(self._utterance("fresh", matchers["fresh"]))
