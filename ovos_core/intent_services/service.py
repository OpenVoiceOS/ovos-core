...
class IntentService:
    ...
    def handle_add_context(message: Message):
        ...
        # Both writes above must land through the shared helper so
        # `sess.intent_context` keeps its dict identity and mutates under
        # the context lock (see `_replace_intent_context`) - #857 was
        # authored against dev, which had no such helper; re-expressing its
        # direct assignment through it here preserves the bus-client
        # identity contract in the merged world.
